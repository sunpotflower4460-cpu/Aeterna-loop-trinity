...
const EXP023_REAL_PARAMS = V212_CANDIDATE_PARAMS;
...
const artifactProvenance = {
  ...
  artifactCommittedIn: 'commit containing this artifact',
};
const observerContextBase = {
  observerVersion: `v212-winding-memory-phase-slip-audit@${generatorGitHash}`,
  generatorGitHash,
...
function commonRecord({ runId, runFamily, runConfig, params, runtime, samples, classification, claimLevelNotes = [], limitations = [] }) {
  ...
  return roundObj({
    runId, runFamily, runConfig, physicsContext: describePhysicsContext(params, runtime.config), observerContext: observerContextBase,
    parameterLineage: PARAMETER_LINEAGE, phaseConstructionMode: PHASE_CONSTRUCTION_MODE, windingInitializationMode: WINDING_INITIALIZATION_MODE, initializerComparisonMode: INITIALIZER_COMPARISON_MODE, runtimeInitializerContext: RUNTIME_INITIALIZER_CONTEXT, samples, ...finalSnap, classification, claimLevelNotes, limitations,
  });
}
...
function perturbationRun({ epsilon, seed, memoryEnabled = true, maxSteps = COST.MAX_STEPS_PERTURBATION, label = 'memory-on', memoryWeight = 0.04 }) {
  const effectiveMemoryWeight = memoryEnabled ? memoryWeight : 0;
  ...
  const rec = commonRecord({ runId: `perturb-W1-e${epsilon}-seed${seed}-${label}`, runFamily: 'perturbation_sweep', runConfig: { initialW: 1, targetW: 1, epsilon, noiseSeed: seed, memoryEnabled, effectiveMemoryWeight, nominalComparisonMemoryWeight: memoryWeight, memoryConditionLabel: label, maxSteps, gridSize: COST.GRID_SIZE }, params, runtime, samples, classification, claimLevelNotes: ['measured: winding loss/recovery, minAmp collapse, and P1 recovery; interpretive tag remains candidate only.'] });
...
function ledgerRun({ label, memoryW, memoryEnabled = true, epsilon = 1.2, seed = 101, memoryWeight = 0.0075 }) {
  ...
  const rec = commonRecord({ runId: `ledger-${label}-mw${memoryWeight}`, runFamily: 'ledger_discrimination_matrix', runConfig: { fieldInitialW: 1, memoryInitialW: memoryW, memoryConditionLabel: label, epsilon, noiseSeed: seed, memoryEnabled, memoryWeight: effectiveMemoryWeight, effectiveMemoryWeight, nominalComparisonMemoryWeight: memoryWeight, maxSteps: COST.MAX_STEPS_PERTURBATION, gridSize: COST.GRID_SIZE }, params, runtime, samples, classification, claimLevelNotes: ['observed ledger-discrimination result; interpretive level: memory_biased_basin_selection if field/memory disagreement changes outcome.'] });
  return { ...rec, fieldInitialW, memoryInitialW: memoryW, memoryConditionLabel: label, epsilon, memoryWeight: effectiveMemoryWeight, effectiveMemoryWeight, nominalComparisonMemoryWeight: memoryWeight, finalFieldDominantW: final.fieldDominantW, finalFieldDominantFraction: final.fieldDominantFraction, finalMemoryDominantW: final.memoryDominantW, finalMemoryDominantFraction: final.memoryDominantFraction, fieldRecoveryStep: analysis.recoveryStep, memoryRewriteStep: memoryRewrite ? memoryRewrite.step : null };
}
...
function pureNoiseRun() {
  const params = baseParams({ MEMORY_ENABLED: false, MEMORY_WEIGHT: 0, MEMORY_COUPLING_ENABLED: false, COUPLING_ENABLED: false }); const runtime = makeRuntime(params); setGlobalWinding(runtime.fieldA, { gridSize: COST.GRID_SIZE, winding: 0, target: 'field' }); copyFieldToMemory(runtime.fieldA); addPerturbation(runtime.fieldA, { epsilon: 1.2, seed: 101 });
  ...
  const rec = commonRecord({ runId: 'pure-noise-W0-e1.2', runFamily: 'pure_noise_control', runConfig: { initialW: 0, targetW: 0, epsilon: 1.2, noiseSeed: 101, memoryEnabled: false, effectiveMemoryWeight: 0, maxSteps: COST.MAX_STEPS_PERTURBATION, gridSize: COST.GRID_SIZE }, params, runtime, samples, classification, claimLevelNotes: ['control: strong W=0 noise should not be described as spontaneous stable W=1 recovery unless measured.'] });
  return { ...rec, finalDominantW: final.fieldDominantW, finalDominantFraction: final.fieldDominantFraction };
}
function couplingRun(g) {
  const params = baseParams({ COUPLING_ENABLED: true, MEMORY_COUPLING_ENABLED: true, MEMORY_COUPLING_USE_BIDIRECTIONAL: true, COUPLING_G: g, MEMORY_WEIGHT: 0.04 });
...
function summarize(results) {
  ...
  return roundObj({ audit: 'v2.1.2-winding-memory-phase-slip-audit', generatedAt: new Date().toISOString(), runMode: COST.mode, parameterLineage: PARAMETER_LINEAGE, ...artifactProvenance, phaseConstructionMode: PHASE_CONSTRUCTION_MODE, windingInitializationMode: WINDING_INITIALIZATION_MODE, initializerComparisonMode: INITIALIZER_COMPARISON_MODE, runtimeInitializerContext: RUNTIME_INITIALIZER_CONTEXT, costPolicy: COST, omittedConditions, observerContext: observerContextBase, modePowerFormula: MODE_POWER_FORMULA, runCount: results.length, byFamily, baseline, priorExpectations, cautions: ['Memory is not interpreted as an infallible ledger.', 'Candidate tags do not imply biological life, consciousness, exact topology proof, or permanent survival.'] });
}
...
function main() {
  ...
  const output = { audit: 'v2.1.2-winding-memory-phase-slip-audit', generatedAt: new Date().toISOString(), runMode: COST.mode, parameterLineage: PARAMETER_LINEAGE, ...artifactProvenance, phaseConstructionMode: PHASE_CONSTRUCTION_MODE, windingInitializationMode: WINDING_INITIALIZATION_MODE, initializerComparisonMode: INITIALIZER_COMPARISON_MODE, runtimeInitializerContext: RUNTIME_INITIALIZER_CONTEXT, results };
