...
function computeEventSummary(run, condition, controlRun) {
  ...
  const guardrailFirst = Object.fromEntries(FIRST_GUARDRAIL_PRECEDENCE.map((name) => [name, firstStep(samples, (s) => s.guardrails[name] === true)]));
  ...
  const couplingNotApplied = condition.couplingG > 0 && !samples.some((s) => s.step > 0 && s.coupling.memoryCouplingApplied && s.coupling.effectiveMemoryCoupling > 0);
...
function main() {
  ...
  const couplingEffectivenessSummary = { coupledRunCount: couplingRuns.length, coupledRunsWithAppliedMemoryCoupling: couplingRuns.filter((r) => !r.eventSummary.couplingNotApplied).length, couplingNotAppliedRunIds: couplingRuns.filter((r) => r.eventSummary.couplingNotApplied).map((r) => r.runId) };
